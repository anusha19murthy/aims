...
class MedicalExtractor:
    ...
    MED_PATTERN = re.compile(
        ...
        r'(?:(od|bd|tds|qid|sos|stat|once daily|twice daily|twice a day|'
        r'three times|thrice daily|at night|hs|morning|evening|'
    ...
    def extract_vitals(self, text):
        ...
        spo2 = self.SPO2_PATTERN.search(text)
        if spo2:
            vitals["spo2"] = spo2.group(1)

        temp = self.TEMP_PATTERN.search(text)
        if temp:
            vitals["temperature"] = temp.group(1)

        weight = self.WEIGHT_PATTERN.search(text)
        if weight:
            vitals["weight"] = weight.group(1)

        rr = self.RR_PATTERN.search(text)
        if rr:
    ...
    def extract_terms(self, text, vocab):
        ...
        for term in vocab:
            if term in text_lower:
                ...
                found.append({"term": term, "negated": negated})
        return found
